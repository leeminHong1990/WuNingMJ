...
// 清一色
cutil.checkIsSameSuit = function(handTiles, upTiles){
	var tile = handTiles[0]
	if (const_val.DRAGONS.indexOf(tile) >= 0 || const_val.WINDS.indexOf(tile) >= 0 || const_val.SEASON.indexOf(tile) >= 0 || const_val.FLOWER.indexOf(tile) >= 0) {
		return false
	}
	for (var i = 0; i < handTiles.length; i++) {
		if (Math.ceil(tile/10) != Math.ceil(handTiles[i]/10)) {
			return false
		}
	}
	for (var i = 0; i < upTiles.length; i++) {
		...
		for (var j = 0; j < meld.length; j++) {
			if (Math.ceil(tile/10) != Math.ceil(meld[j]/10)) {
				return false
			}
		}
	}
...
// 杠的牌 分类
cutil.getKongClassify = function(upTilesOpsList){
	var classifyList = [0, 0, 0, 0]
	for (var i = 0; i < upTilesOpsList.length; i++) {
		var cell_list = upTilesOpsList[i]
		var cell = cell_list[1] ? cell_list[1] : cell_list[0]

		if (cell['opId'] == const_val.OP_EXPOSED_KONG) {
			if (cell["tiles"][0] == const_val.DRAGON_RED || cell["tiles"][0] == const_val.DRAGON_WHITE) {
				classifyList[2] += 1
			} else {
				classifyList[0] += 1
			}
		} else if (cell['opId'] == const_val.OP_CONCEALED_KONG) {
			if (cell["tiles"][0] == const_val.DRAGON_RED || cell["tiles"][0] == const_val.DRAGON_WHITE) {
				classifyList[3] += 1
			} else {
				classifyList[1] += 1
...
//碰碰胡
cutil.checkIsPongPongWin = function(handTiles, upTiles){
	for (var i = 0; i < upTiles.length; i++) {
		var meld = upTiles[i]
	...
	var tile2NumDict = cutil.getTileNumDict(handTiles)
	for (var key in tile2NumDict) {
		if (tile2NumDict[key] != 3 && tile2NumDict[key] != 2) {
			return false
		}
	}
	return true
...
cutil.checkIsKongDrawWin = function(p_op_r){
	for (var i = p_op_r.length - 1; i >= 0; i--) {
		var cell = p_op_r[i][1] ? p_op_r[i][1] : p_op_r[i][0]
		if (cell["opId"] == const_val.OP_DRAW || cell["opId"] == const_val.OP_CUT) {
			continue
		}
		if (cell["opId"] == const_val.OP_CONCEALED_KONG) {
			return true
		} else if (cell["opId"] == const_val.OP_EXPOSED_KONG) {
			return true
		}
		return false
	}
...
// 杠后打牌
cutil.checkIsKongDiscard = function(p_op_r){
	if (p_op_r.length <= 0) {
		return false
	}
	if (p_op_r[p_op_r.length-1][0]["opId"] == const_val.OP_DISCARD) {
		for (var i = p_op_r.length - 2; i >= 0; i--) {
			var cell = p_op_r[i][1] ? p_op_r[i][1] : p_op_r[i][0]
			if (cell["opId"] == const_val.OP_DRAW || cell["opId"] == const_val.OP_CUT) {
				continue;
			}
