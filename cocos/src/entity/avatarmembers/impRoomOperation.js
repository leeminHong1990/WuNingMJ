...
var impRoomOperation = impGameRules.extend({
	...
	createRoom : function(player_num, round_num, lucky_num, extra_score, is_agent) {
		cc.log("createRoom:",player_num, round_num, lucky_num, extra_score, is_agent)
		this.baseCall("createRoom", player_num, round_num, lucky_num, extra_score, is_agent);
	},
	...
	enterRoomFailed : function(err){
		...
		if(err == -1){
            ...
            h1global.globalUIMgr.info_ui.show_by_info("进入房间失败，钻石不足！", cc.size(300, 200));
        }
	},
	...
	// notifyPlayerGiveUp : function(){
	// 	cc.log("notifyPlayerGiveUp")
	// 	this.curGameRoom.giveUpState = const_val.WAIT_GIVE_UP
	// 	if(h1global.curUIMgr.gameroom_ui && h1global.curUIMgr.gameroom_ui.is_show){
	// 		h1global.curUIMgr.gameroom_ui.show_give_up_panel()
	// 	}
	// },

	// confirmGiveUp : function(isGiveUp){
	// 	this.curGameRoom.giveUpState = isGiveUp
	// 	this.baseCall("confirmGiveUp", isGiveUp)
	// },

	// unlockPongPlayerOperation:function(){
	// 	cc.log("unlockPongPlayerOperation")
	// 	if(h1global.curUIMgr.gameroom_ui && h1global.curUIMgr.gameroom_ui.is_show){
