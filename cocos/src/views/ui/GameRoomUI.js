...
var GameRoomUI = UIBase.extend({
	...
	initUI:function(){
        ...
        for (var i = 0; i < 4; i++) {
        	this.rootUINode.getChildByName("draw_tile_panel" + i.toString()).setVisible(false);
        }
	...
	init_player_tile_panel:function(){
		...
		for(var i = 0; i < player.curGameRoom.player_num; i++){
			...
			this.update_player_dragon_green_tiles(i);
		}
	},
	...
	startBeginAnim:function(diceList, dealerIdx){
		...
		function setBeginGameShow(is_show){
			...
			for (var i = 0; i < player.curGameRoom.player_num; i++) {
				...
				var player_dragon_green_panel = self.rootUINode.getChildByName("player_tile_panel" + idx.toString()).getChildByName("player_dragon_green_panel");
				player_dragon_green_panel.setVisible(is_show)
			...
			self.rootUINode.runAction(cc.Repeat.create(
				cc.Sequence.create(cc.DelayTime.create(0.1),
					cc.CallFunc.create(function(){
						...
						cc.log(cur_tile_num)
						if(cur_tile_num == player.curGameRoom.handTilesList[player.serverSitNum].length){
							playAnimationEnd()
						}
						for(var i = 0; i < player.curGameRoom.player_num; i++){
							self.update_player_hand_tiles(i, player.curGameRoom.handTilesList[i].slice(0, cur_tile_num));
				...
				), player.curGameRoom.handTilesList[player.serverSitNum].length
			));
		}
  ...
  //   	var curPlayNum = 0
	...
	update_player_dragon_green_tiles:function(serverSitNum){
		if(!this.is_show) {return;}

		var player = h1global.entityManager.player();
		var idx = player.server2CurSitNum(serverSitNum);
		cc.log(serverSitNum, player.curGameRoom.dragonGreenList)
		var player_dragon_green_panel = this.rootUINode.getChildByName("player_tile_panel" + idx.toString()).getChildByName("player_dragon_green_panel");
		for (var i = 0; i < 4; i++) {
			var tile_img = player_dragon_green_panel.getChildByName("tile_img_" + i.toString());
			if (player.curGameRoom.dragonGreenList[serverSitNum][i]) {
				var mahjong_img = tile_img.getChildByName("mahjong_img");
				mahjong_img.ignoreContentAdaptWithSize(true);
				mahjong_img.loadTexture("Mahjong/mahjong_small_" + const_val.DRAGON_GREEN.toString() + ".png", ccui.Widget.PLIST_TEXTURE);
				tile_img.setVisible(true)
				continue;
			}
			tile_img.setVisible(false)
		...
		if(idx == 0){
			...
			for(var i = 0; i < player.curGameRoom.upTilesList[serverSitNum].length; i++){
				...
				if(player.curGameRoom.upTilesList[serverSitNum][i].length > 3){
					...
					if(player.curGameRoom.upTilesList[serverSitNum][i][2] == 0){
						kong_tile_img.loadTexture("Mahjong/" + mahjong_down_str, ccui.Widget.PLIST_TEXTURE);
						mahjong_img.setVisible(false);
					}
	...
	init_operation_panel:function(){
		...
		this.operation_panel.getChildByName("op_btn5").addTouchEventListener( //杠单张发财
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_DRAGON_GREEN_KONG, [player.curGameRoom.lastDrawTile]);
					}else{
						player.doOperation(const_val.OP_DRAGON_GREEN_KONG, [player.curGameRoom.lastDrawTile]);
					}
		...
		this.operation_panel.getChildByName("op_btn8").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_WIN, [0]);
					} else {
		...
		if(player.curGameRoom.waitAid >= 0){
            ...
            if (player.curGameRoom.lastDrawTile == const_val.DRAGON_GREEN) { //抓到的是 发财
                op_list.push(const_val.OP_DRAGON_GREEN_KONG)
                this.lock_player_hand_tiles();
            }
            if(op_list.length == 0) {
                // 是否可以明杠
			...
			// 轮到自己摸牌, 不一定可以进行打牌操作
			if (player.curGameRoom.lastDrawTile != const_val.DRAGON_GREEN) {
				this.unlock_player_hand_tiles();
			}
		}
	},
	...
	update_ready_tile_panel:function(readyTiles){
		...
		if(readyTiles.length <= 0){
			...
			// }
			// this.readyTileList = [];
			this.ready_tile_panel.setContentSize(cc.size(100 + 40 * readyTiles.length, 41));
	...
	playOperationEffect:function(opId, serverSitNum, tile){
		...
		if(opId == const_val.OP_PONG){
		...
		} else if(opId == const_val.OP_DRAGON_GREEN_KONG){
			// if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
			// 	UICommonWidget.load_effect_plist("effect_kong");
			// 	var effect_sprite = cc.Sprite.create();
			// 	if(curSitNum == 0){
			...
			// 	));
			// } else {
				cur_img.loadTexture("res/ui/GameRoomUI/gameroom_op_fa.png");
				cur_img.setScale(5.0);
				if(curSitNum == 0){
					cur_img.setPosition(cc.p(cc.winSize.width * 0.5, this.rootUINode.getChildByName("player_tile_panel0").getPositionY()+160));
				} else if(curSitNum == 1){
			...
			if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
				...
				this.rootUINode.addChild(effect_sprite);
                // effect_sprite.setLocalZOrder(99);
				effect_sprite.runAction(cc.Sequence.create(
	...
	play_luckytiles_anim:function(luckyTileList, callback){
		...
		var show_lucky_tiles_panels = this.rootUINode.getChildByName("show_lucky_tiles_panel");
        if(lucky_num == 0){
            show_lucky_tiles_panels.runAction(cc.Sequence.create(cc.DelayTime.create(1.2), cc.CallFunc.create(function(){
                if(callback){callback();}
            })));
            return;
        }
        show_lucky_tiles_panels.setVisible(true);
		for(var idx = 0; idx < luckyTileList.length; idx++) {
            if (luckyTileList[idx].length > 0) {
                var show_lucky_tiles_panel = show_lucky_tiles_panels.getChildByName("show_lucky_tiles_panel_" + h1global.entityManager.player().server2CurSitNum(idx));
                var panel_width = 160 * lucky_num;
                show_lucky_tiles_panel.setContentSize(cc.size(panel_width, show_lucky_tiles_panel.getContentSize().height));
                show_lucky_tiles_panel.setScale(0.5);
                ...
                for (var i = 0; i < lucky_num; i++) {
                    ...
                    var luckyTileNum = luckyTileList[idx][i];
                    if (luckyTileNum) {
                ...
                if(lucky_num == 1){
                    show_lucky_tiles_panel.getChildByName("tile_img1").setVisible(false);
                }
            }
        }
		// 由于同时会播放胡牌特效，所以总时间至少2s
		show_lucky_tiles_panels.runAction(cc.Sequence.create(cc.DelayTime.create(luckyTileList.length * 0.4 + 0.4), cc.CallFunc.create(function(){
			if(callback){callback();}
		})));
	},
	...
	update_kingtile_panel:function(){
		cc.log("update_kingtile_panel===> false")
		this.rootUINode.getChildByName("kingtile_panel").setVisible(false)
		return
		// if(!this.is_show){
		// 	return;
		// }
