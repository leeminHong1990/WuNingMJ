...
class iRoomOperation(object):
	...
	def createRoom(self, player_num, game_round, lucky_num, extra_score, is_agent):
		DEBUG_MSG("create room, {0}, {1}, {2}, {3}, {4}".format(player_num, game_round, lucky_num, extra_score, is_agent))
		if game_round <= 0:
			return
		...
		if room:
			...
			if room.is_agent == 1:
				...
				if switch.DEBUG_BASE:
					...
					utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost ,callback, "WuNingMJ RoomID:{}".format(self.roomID)) # reason 必须为英文
			else:
				room.reqEnterRoom(self, True)
				self.room = room
				KBEngine.globalData["GameWorld"].addRoom(room)
	...
	def startGame(self, dealer_idx, tile_list, dragon_greens_list, kingTiles, diceList):
		if getattr(self, 'client', None):
			self.client.startGame(dealer_idx, tile_list, dragon_greens_list, kingTiles, diceList)
