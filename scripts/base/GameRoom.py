...
class GameRoom(KBEngine.Base, GameObject, iRoomRules):
	...
	def drawAfterKongDG(self, drawNum):
		if len(self.tiles) <=  drawNum:
			self.drawEnd()
			return
		player = self.players_list[self.current_idx]
		tile_list = self.tiles[0:drawNum]
		self.tiles = self.tiles[drawNum:]
		DEBUG_MSG("drawAfterKongDG len:{0}".format(len(self.tiles)))
		player.drawTile(tile_list)
	...
	def winGame(self, idx, op, quantity, result):
		...
		for i,p in enumerate(self.players_list):
			if p is not None:
				if i == idx:
					DEBUG_MSG("winGame add score add {0}:{1}:{2}".format(i, quantity, (quantity//4 + 1) * (self.player_num - 1)))
					p.addScore((quantity//4 + 1) * (self.player_num - 1))
				else:
					DEBUG_MSG("winGame add score sub {0}:{1}:{2}".format(i, quantity, -(quantity//4 + 1)))
					p.addScore(-(quantity//4 + 1))
		""" lucky_tile"""
		luckyTiles = self.getLuckyTiles([idx])
		self.cal_lucky_tile_score(luckyTiles, op)
		self.cal_extra_score([idx], op)
	...
	def cal_lucky_tile_score(self, lucky_tiles, win_op, formIdx = 0):
		DEBUG_MSG('lucky_tiles {}'.format(lucky_tiles))
		if win_op == const.OP_DRAW_WIN:
			idx = 0
			val = 0
			for i in range(len(lucky_tiles)):
				for tile in lucky_tiles[i]:
					idx = i
					if tile < 30 and (tile % 10 == 1 or tile % 10 == 5 or tile % 10 == 9):
						val += 1
			for i,p in enumerate(self.players_list):
				if p is not None:
					if i == idx:
						DEBUG_MSG("cal_lucky_tile_score draw win add {0}:{1}".format(i, val * (self.player_num-1)))
						p.addScore(val * (self.player_num-1))
					else:
						DEBUG_MSG("cal_lucky_tile_score draw win sub {0}:{1}".format(i, -val))
						p.addScore(-val)
		else:
			...
			for i in range(len(lucky_tiles)):
				...
				for tile in lucky_tiles[i]:
					if tile < 30 and (tile % 10 == 1 or tile % 10 == 5 or tile % 10 == 9):
						val += 1
				if val > 0:
					DEBUG_MSG("cal_lucky_tile_score give win add {0}:{1}".format(i, val))
					DEBUG_MSG("cal_lucky_tile_score give win sub {0}:{1}".format(formIdx, -val))
					self.players_list[i].addScore(val)
					self.players_list[formIdx].addScore(-val)

	def cal_extra_score(self, winIdxList, win_op, formIdx = 0):
		if win_op == const.OP_DRAW_WIN:
			for i,p in enumerate(self.players_list):
				if p is not None:
					if i in winIdxList:
						DEBUG_MSG("cal_extra_score draw win add {0}:{1}".format(i, 2 * self.extra_score * (self.player_num-1)))
						p.addScore(2 * self.extra_score * (self.player_num-1))
					else:
						DEBUG_MSG("cal_extra_score draw win sub {0}:{1}".format(i, -2 * self.extra_score))
						p.addScore(-2 * self.extra_score)
		else:
			for i in winIdxList:
				if i < len(self.players_list):
					DEBUG_MSG("cal_extra_score give win add {0}:{1}".format(i, 2 * self.extra_score))
					DEBUG_MSG("cal_extra_score give win sub {0}:{1}".format(formIdx, -2 * self.extra_score))
					self.players_list[i].addScore(2 * self.extra_score)
					self.players_list[formIdx].addScore(-2 * self.extra_score)
	...
	def doOperation(self, avt_mb, aid, tile_list):
		...
		if aid == const.OP_DISCARD and self.can_discard(p.tiles, tile):
		...
		elif aid == const.OP_DRAGON_GREEN_KONG and self.check_is_dragon_green(tile_list) and len(tile_list) <= self.get_dragon_green_num(p.tiles):
			p.kongDragonGreen(tile_list)
			...
			if is_win:
				p.draw_win(aid, quantity, result)
			else:
	...
	def confirmOperation(self, avt_mb, aid, tile_list):
		...
		self.wait_idx_list.remove(idx)
		if len(self.wait_idx_list) <= 0:
			self.wait_op = None
		p = self.players_list[idx]
		if aid == const.OP_PONG and self.can_pong(p.tiles, tile, idx):
			...
			self.current_idx = idx
			p.chow(tile_list)
		elif aid == const.OP_KONG_WIN or aid == const.OP_GIVE_WIN:
			if len(self.wait_for_win_dict) > 0:
				...
				avt_mb.doOperationFailed(const.OP_ERROR_ILLEGAL)
				#无任何操作 过牌
				self.confirmPass(idx, -1)
		else:
			#无任何操作 过牌
			#设置玩家 确认胡状态
			if idx in self.wait_for_win_dict:
				self.wait_for_win_dict[idx]["state"] = 2 #state 0 未确认 1确认胡 2不胡
				self.players_list[idx].pass_dealer[0] = 1
			
			tile = self.all_discard_tiles[-1]
				...
				if temp_op == const.OP_GIVE_WIN:
					for i, p in enumerate(self.players_list):
						if p and i != self.last_player_idx:
							...
							if self.can_pong(p.tiles, tile, i) or self.can_exposed_kong(p.tiles, tile):
	...
	def confirmPass(self, idx, last_tile):
		if last_tile > 0 and last_tile not in self.players_list[idx].pass_dealer[1]:
			self.players_list[idx].pass_dealer[1].append(last_tile)
		nextIdx = self.nextIdx
		nextMb = self.players_list[nextIdx]
		if idx != nextIdx and last_tile > 0 and self.can_chow(nextMb.tiles, last_tile):
			self.wait_idx_list = [nextIdx]
			self.wait_op = const.OP_DISCARD
			nextMb.mb.waitForOperation(const.OP_DISCARD, [last_tile,])
		else:
	...
	def processMutiWin(self, win_list, quantity_list, result_list, win_op, formIdx_list, tile):
		...
		op_list = [win_op] * len(win_idx_list)
		self.broadcastMultiOperation(win_idx_list, op_list, win_op, [tile,])

		#结算
		for i in range(len(win_list)):
			fromIdx = formIdx_list[i]
			DEBUG_MSG("processMutiWin sub score {0}:{1}:{2}".format(fromIdx, quantity_list[i], -(quantity_list[i]//4 + 1)))
			DEBUG_MSG("processMutiWin add score {0}:{1}:{2}".format(i, quantity_list[i], quantity_list[i]//4 + 1))
			self.players_list[fromIdx].addScore(-(quantity_list[i]//4 + 1))
			win_list[i].addScore(quantity_list[i]//4 + 1)

		luckyTiles = self.getLuckyTiles(win_idx_list)
		self.cal_lucky_tile_score(luckyTiles, win_op, formIdx_list[0])
		self.cal_extra_score(win_idx_list, win_op, formIdx_list[0])

		if len(win_list) >= 2:
			self.dealer_idx = formIdx_list[0]
		elif self.dealer_idx not in win_idx_list:
	...
	def waitForOperation(self, idx, aid, tile):
		isHaveWaitOp = False
		if aid == const.OP_EXPOSED_KONG or aid == const.OP_CONCEALED_KONG:	#玩家普通杠 是否可以抢杠胡
			self.wait_idx_list = []
			self.wait_for_win_dict = {}
			...
			for i,p in enumerate(player_list):
				if p is not None:
					...
					DEBUG_MSG("wait for kong win:{0}".format(wait_idx))
		elif aid == const.OP_DISCARD:
			self.wait_idx_list = []
			self.wait_for_win_dict = {}
			player_list, quantity_list, result_list = self.checkGiveWin(idx, tile)
			if player_list:
				for i,p in enumerate(player_list):
					if p is not None:
						wait_idx = self.players_list.index(p)
						...
						self.wait_for_win_dict[wait_idx] = {"state": 0, "formIdx": idx, "win_op":const.OP_GIVE_WIN, "quantity":quantity_list[i], "result":result_list[i]}
						isHaveWaitOp = True
						p.mb.waitForOperation(const.OP_GIVE_WIN, [tile,])
						DEBUG_MSG("wait for give win:{0}".format(wait_idx))	
			else:
				for i, p in enumerate(self.players_list):		#玩家打牌 是否有其他玩家碰 杠
	...
	def checkGiveWin(self, idx, tile):
		wait_player_list = []
		quantity_list = []
		result_list = []
		for i, p in enumerate(self.players_list):
			...
			tmp = list(p.tiles)
			tmp.append(tile)
			is_win, quantity, result = self.can_win(tmp, tile, const.OP_GIVE_WIN, i)
			if is_win:
				wait_player_list.append(p)
				quantity_list.append(quantity)
				result_list.append(result)
